        ...
        if (Physics.SphereCast(ray, 1f, out hit))
        {
            ...
            if (playerCharacter.GetComponent<CharacterController>())
            {
                if (_bullet == null)
                {
                    ...
                    Vector3 directionToPlayer = playerCharacter.transform.position - _bullet.transform.position;

                    _bullet.transform.rotation = Quaternion.LookRotation(directionToPlayer);
                }
            }
        }
